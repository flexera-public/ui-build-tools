...
function generate(bundle, minify) {
  let preBuildDeps = _.map(bundle.dependencies, d => d + ':preBuild');
  let buildDeps = [];
  let compileDeps = [];
  ...
  if (bundle.angular || typeof bundle.angular === 'undefined') {
    ...
    let svgFiles = glob.sync(`${root}**/*.svg`, { ignore: `${root}/assets/**/*` });
    if (svgFiles.length) {
      // If any, add images pre-cache task
      let taskName = `${bundle.name}:images`;
      gulp.task(taskName, () => images.preCache([`${root}**/*.svg`, `!${root}assets/**/*.svg`], bundle.name));
      preBuildDeps.push(taskName);
    }
  }

  if (fs.existsSync(`${root}assets`)) {
    let taskName = `${bundle.name}:assets`;
    gulp.task(taskName, () => {
      return gulp.src(`${root}assets/**/*`)
        .pipe(gulp.dest(`${ bundle.dist ? 'dist' : 'build' }/assets`));
    });
  ...
  if (fs.existsSync(`${root}index.ts`)) {
    ...
    gulp.task(verifyName, preBuildDeps, () =>
      code.verify([`${root}**/*.ts`, `!${root}spec.ts`, `!${root}**/*.spec.ts`, `!${root}spec/**/*`])
    );
  ...
  gulp.task(`${bundle.name}:build`, preBuildDeps.concat(buildDeps), () => {
